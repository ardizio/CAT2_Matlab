...
uint16_t encAngle_1 = 0, encAngle_2 = 0;
...
void loop(){
  ...
  if(state != 0){
    ...
    if (encoderPosition != 0xFFFF){
      ang_3 = encoderPosition / 4096.0*360.0;
    }
    ...
    prev_angle = ang_3;
    cart_position = (intAngle + ang_3)*CONST;
  }
  ...
  if(readVal >= 0x4000){
    ...
    encAngle_1 = readVal - 15;
    ang_1 = encAngle_1 / 4096.0*360.0;
  }

  // Time Handling
  ttNow = millis();
  deltaT = (ttNow - tt)/1000.0;
  tt = ttNow;
  ...
  switch(state){
    ...
    case CTRL:{
      ...
      u[0]=28*u_angle[1]-55.6885*u_angle[2]+27.6886*u_angle[3]-1*e_angle[0]+1.9763*e_angle[1]-0.97629*e_angle[2];
